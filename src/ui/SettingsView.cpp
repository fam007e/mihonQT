...
SettingsView::SettingsView(QWidget *parent) : QWidget(parent) {
  ...
  connect(&ThemeManager::instance(), &ThemeManager::themeChanged, this,
          &SettingsView::updateThemeColors);
...
void SettingsView::setupUi() {
  ...
  connect(m_categoryList, &QListWidget::currentRowChanged, this,
          &SettingsView::onCategoryChanged);
...
void SettingsView::selectCategory(const QString &categoryName) {
...
void SettingsView::updatePreferences() {
  // Sync Incognito
  bool incognito =
      PreferenceManager::instance().value("security/incognito", false).toBool();
  if (m_incognitoCheckBox && m_incognitoCheckBox->isChecked() != incognito) {
    m_incognitoCheckBox->blockSignals(true);
...
void SettingsView::updateThemeColors() {
  ...
  QWidget *logoCircle = findChild<QWidget *>("aboutLogoCircle");
  if (logoCircle) {
    logoCircle->setStyleSheet(QString("QWidget#aboutLogoCircle { "
                                      "  background-color: %1; "
                                      "  border-radius: 60px; "
                                      "}")
                                  .arg(accentColor));
  ...
  QLabel *logoChar = findChild<QLabel *>("aboutLogoChar");
  if (logoChar) {
    logoChar->setStyleSheet(
        QString("font-size: 61px; font-weight: bold; color: %1; background: "
                "transparent; margin-bottom: -0px;")
            .arg(windowColor));
  }

  QLabel *logoBadge = findChild<QLabel *>("aboutLogoBadge");
  if (logoBadge) {
...
void SettingsView::createLibraryPage() {
  ...
  // Load saved value
  int savedInterval =
      PreferenceManager::instance().value("library/update_interval", 0).toInt();
  int index = m_updateIntervalComboBox->findData(savedInterval);
  if (index != -1)
    m_updateIntervalComboBox->setCurrentIndex(index);

  connect(m_updateIntervalComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
            int value = m_updateIntervalComboBox->itemData(index).toInt();
            PreferenceManager::instance().setValue("library/update_interval",
                                                   value);
          });

  m_autoUpdateMetadataCheckBox = new QCheckBox("Refresh Metadata", this);
  m_autoUpdateMetadataCheckBox->setChecked(
      PreferenceManager::instance()
          .value("library/auto_update_metadata", false)
          .toBool());
  connect(m_autoUpdateMetadataCheckBox, &QCheckBox::toggled, [](bool checked) {
    PreferenceManager::instance().setValue("library/auto_update_metadata",
                                           checked);
  });
  ...
  connect(m_editCategoriesButton, &QPushButton::clicked, this,
          &SettingsView::onEditCategoriesClicked);
  ...
  QString savedStartAction =
      PreferenceManager::instance()
          .value("library/swipe_start_action", "disabled")
          .toString();
  index = m_swipeStartActionComboBox->findData(savedStartAction);
  if (index != -1)
    m_swipeStartActionComboBox->setCurrentIndex(index);

  connect(m_swipeStartActionComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
            QString value =
                m_swipeStartActionComboBox->itemData(index).toString();
            PreferenceManager::instance().setValue("library/swipe_start_action",
                                                   value);
          });
  ...
  QString savedEndAction = PreferenceManager::instance()
                               .value("library/swipe_end_action", "disabled")
                               .toString();
  index = m_swipeEndActionComboBox->findData(savedEndAction);
  if (index != -1)
    m_swipeEndActionComboBox->setCurrentIndex(index);

  connect(m_swipeEndActionComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
            QString value =
                m_swipeEndActionComboBox->itemData(index).toString();
            PreferenceManager::instance().setValue("library/swipe_end_action",
                                                   value);
          });
...
void SettingsView::createReaderPage() {
  ...
  int savedMode =
      PreferenceManager::instance().value("reader/default_mode", 0).toInt();
  int index = m_readingModeComboBox->findData(savedMode);
  if (index != -1)
    m_readingModeComboBox->setCurrentIndex(index);

  connect(m_readingModeComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
            int value = m_readingModeComboBox->itemData(index).toInt();
            PreferenceManager::instance().setValue("reader/default_mode",
                                                   value);
            emit readingModeChanged(value);
            // Force reader to reload settings if active
            // Ideally we'd have a signal for this, but for now we rely on
            // ReaderWidget checking prefs or being re-created Actually, let's
            // emit a generic signal or have ReaderWidget listen to something.
            // For this task, we will just ensure ReaderWidget reloads when
            // shown/created. But for immediate effect, we need a signal.
          });
  ...
  m_fullscreenCheckBox->setChecked(
      PreferenceManager::instance().value("reader/fullscreen", false).toBool());
  ...
  m_keepScreenOnCheckBox->setChecked(PreferenceManager::instance()
                                         .value("reader/keep_screen_on", false)
                                         .toBool());
  ...
  m_showPageNumberCheckBox->setChecked(
      PreferenceManager::instance()
          .value("reader/show_page_number", true)
          .toBool());
  ...
  m_pageTransitionsCheckBox->setChecked(
      PreferenceManager::instance()
          .value("reader/page_transitions", true)
          .toBool());
  ...
  int savedSpeed = PreferenceManager::instance()
                       .value("reader/double_tap_speed", 500)
                       .toInt();
  index = m_doubleTapAnimSpeedComboBox->findData(savedSpeed);
  if (index != -1)
    m_doubleTapAnimSpeedComboBox->setCurrentIndex(index);

  connect(m_doubleTapAnimSpeedComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
            int value = m_doubleTapAnimSpeedComboBox->itemData(index).toInt();
            PreferenceManager::instance().setValue("reader/double_tap_speed",
                                                   value);
          });
  ...
  int savedScale =
      PreferenceManager::instance().value("reader/scale_type", 1).toInt();
  index = m_scaleTypeComboBox->findData(savedScale);
  if (index != -1)
    m_scaleTypeComboBox->setCurrentIndex(index);

  connect(m_scaleTypeComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
  ...
  int savedZoom =
      PreferenceManager::instance().value("reader/zoom_start", 1).toInt();
  index = m_zoomStartComboBox->findData(savedZoom);
  if (index != -1)
    m_zoomStartComboBox->setCurrentIndex(index);

  connect(m_zoomStartComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
  ...
  m_cropBordersCheckBox->setChecked(PreferenceManager::instance()
                                        .value("reader/crop_borders", false)
                                        .toBool());
  ...
  int savedPadding =
      PreferenceManager::instance().value("reader/webtoon_padding", 0).toInt();
  index = m_webtoonSidePaddingComboBox->findData(savedPadding);
  if (index != -1)
    m_webtoonSidePaddingComboBox->setCurrentIndex(index);

  connect(m_webtoonSidePaddingComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          [this](int index) {
            int value = m_webtoonSidePaddingComboBox->itemData(index).toInt();
            PreferenceManager::instance().setValue("reader/webtoon_padding",
                                                   value);
          });
...
void SettingsView::createDownloadsPage() {
  ...
  m_downloadNewChaptersCheckBox->setChecked(
      PreferenceManager::instance()
          .value("downloads/download_new", false)
          .toBool());
  ...
  m_deleteRemovedChaptersCheckBox =
      new QCheckBox("Delete chapters after reading", this);
  m_deleteRemovedChaptersCheckBox->setChecked(
      PreferenceManager::instance()
          .value("downloads/delete_removed", false)
          .toBool());
  connect(m_deleteRemovedChaptersCheckBox, &QCheckBox::toggled,
          [](bool checked) {
            PreferenceManager::instance().setValue("downloads/delete_removed",
                                                   checked);
          });
...
void SettingsView::createTrackingPage() {
  ...
  m_autoUpdateTrackersCheckBox =
      new QCheckBox("Automatically update trackers", this);
  m_autoUpdateTrackersCheckBox->setChecked(
      PreferenceManager::instance()
          .value("tracking/auto_update", true)
          .toBool());
...
void SettingsView::createBrowsePage() {
  ...
  m_checkForSourceUpdatesCheckBox =
      new QCheckBox("Check for source updates", this);
  m_checkForSourceUpdatesCheckBox->setChecked(
      PreferenceManager::instance()
          .value("browse/check_updates", true)
          .toBool());
  connect(
      m_checkForSourceUpdatesCheckBox, &QCheckBox::toggled, [](bool checked) {
        PreferenceManager::instance().setValue("browse/check_updates", checked);
      });

  m_autoUpdateExtensionsCheckBox =
      new QCheckBox("Auto-update extensions", this);
  m_autoUpdateExtensionsCheckBox->setChecked(
      PreferenceManager::instance()
          .value("browse/auto_update_extensions", false)
          .toBool());
  connect(m_autoUpdateExtensionsCheckBox, &QCheckBox::toggled,
          [](bool checked) {
            PreferenceManager::instance().setValue(
                "browse/auto_update_extensions", checked);
          });

  m_showNsfwSourceCheckBox = new QCheckBox("Show NSFW sources", this);
  m_showNsfwSourceCheckBox->setChecked(
      PreferenceManager::instance().value("browse/show_nsfw", true).toBool());
  ...
  connect(m_localSourceLocationButton, &QPushButton::clicked, this, [this]() {
    QString dir = QFileDialog::getExistingDirectory(
        this, "Select Local Manga Directory", QDir::homePath(),
...
void SettingsView::createDataStoragePage() {
  ...
  connect(m_createBackupButton, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getSaveFileName(
        this, "Create Backup", BackupManager::getDefaultBackupPath(),
        "JSON Files (*.json)");
    if (!path.isEmpty()) {
      if (BackupManager::instance().exportBackup(path)) {
        QMessageBox::information(this, "Success",
  ...
  connect(m_restoreBackupButton, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getOpenFileName(
        this, "Restore Backup", BackupManager::getDefaultBackupPath(),
        "JSON Files (*.json)");
    if (!path.isEmpty()) {
      if (BackupManager::instance().importBackup(path)) {
        QMessageBox::information(this, "Success",
                                 "Backup restored successfully. Some changes "
  ...
  connect(m_clearCacheButton, &QPushButton::clicked, this, [this]() {
    if (QMessageBox::question(
            this, "Clear Cache",
            "Are you sure you want to clear the chapter cache?") ==
        QMessageBox::Yes) {
      // Logic to clear cache directory
      QMessageBox::information(this, "Success", "Chapter cache cleared.");
    }
  });

  m_clearCookiesButton = new QPushButton("Clear Cookie Jar", this);
  connect(m_clearCookiesButton, &QPushButton::clicked, this, [this]() {
    if (QMessageBox::question(this, "Clear Cookies",
                              "Are you sure you want to clear all cookies?") ==
        QMessageBox::Yes) {
      // Logic to clear cookies
      QMessageBox::information(this, "Success", "Cookies cleared.");
    }
  });
  ...
  QLabel *dataInfoLabel =
      new QLabel("Location for extensions, downloads, and other data.", this);
  ...
  mDataDirectoryEdit->setText(PreferenceManager::instance()
                                  .value("dataDirectory", defaultDataDir)
                                  .toString());

  mDataDirectoryButton = new QPushButton("Browse...", this);
  connect(mDataDirectoryButton, &QPushButton::clicked, this, [this]() {
    QString dir = QFileDialog::getExistingDirectory(
        this, "Select Data Directory", mDataDirectoryEdit->text());
    if (!dir.isEmpty()) {
      ...
      QMessageBox::information(this, "Restart Required",
                               "Please restart MihonQT for data directory "
...
void SettingsView::createSecurityPage() {
  ...
  m_incognitoCheckBox = new QCheckBox("Incognito Mode", this);
  m_incognitoCheckBox->setChecked(PreferenceManager::instance()
                                      .value("security/incognito", false)
                                      .toBool());
  connect(m_incognitoCheckBox, &QCheckBox::toggled, this, [this](bool checked) {
    PreferenceManager::instance().setValue("security/incognito", checked);
    emit incognitoChanged(checked);
  });

  m_secureScreenCheckBox = new QCheckBox("Secure Screen", this);
  m_secureScreenCheckBox->setChecked(PreferenceManager::instance()
                                         .value("security/secure_screen", false)
                                         .toBool());
  connect(m_secureScreenCheckBox, &QCheckBox::toggled, [](bool checked) {
...
void SettingsView::createAdvancedPage() {
  ...
  m_clearDbButton = new QPushButton("Clear Database", this);
  connect(m_clearDbButton, &QPushButton::clicked, this, [this]() {
    if (QMessageBox::question(
            this, "Clear Database",
            "Are you sure you want to clear the entire database? This will "
            "delete all library data, history, and categories.") ==
        QMessageBox::Yes) {
      // Logic to clear database
      QMessageBox::information(this, "Success",
                               "Database cleared. Please restart MihonQT.");
    }
  });

  m_dumpLogsButton = new QPushButton("Dump Crash Logs", this);
  connect(m_dumpLogsButton, &QPushButton::clicked, this, [this]() {
    // Logic to dump logs
    QMessageBox::information(this, "Success", "Crash logs dumped to desktop.");
  });

  advancedLayout->addWidget(m_clearDbButton);
  advancedLayout->addWidget(m_dumpLogsButton);
...
void SettingsView::createAboutPage() {
  ...
  // App Logo (Matching MoreView)
  QWidget *logoCircle = new QWidget(page);
  logoCircle->setFixedSize(120, 120);
  logoCircle->setObjectName("aboutLogoCircle");
  ...
  QLabel *logoChar = new QLabel("み", logoCircle);
  logoChar->setObjectName("aboutLogoChar");
  logoChar->setAlignment(Qt::AlignCenter);

  QLabel *logoBadge = new QLabel("QT", logoCircle);
  logoBadge->setObjectName("aboutLogoBadge");
  logoBadge->setAlignment(Qt::AlignCenter);

  circleLayout->addWidget(logoChar);
  circleLayout->addWidget(logoBadge);
  ...
  titleLabel->setStyleSheet(
      "font-size: 28px; font-weight: bold; color: #ECEFF4;");
  ...
  QLabel *descLabel =
      new QLabel("A powerful, cross-platform manga reader for desktop.<br>"
  ...
  linksLayout->addWidget(
      createLink("GitHub", "https://github.com/fam007e/mihonQT"));
  linksLayout->addWidget(
      createLink("Website", "https://fam007e.github.io/mihonQT/"));
  linksLayout->addWidget(
      createLink("FAQ", "https://fam007e.github.io/mihonQT/FAQ.html"));
  layout->addLayout(linksLayout);

  layout->addStretch();

  QLabel *footerLabel =
      new QLabel("© 2025 MihonQT Contributors. Licensed under AGPLv3.", this);
  footerLabel->setStyleSheet(
      "font-size: 11px; color: #4C566A; padding-bottom: 20px;");
...
void SettingsView::onCategoryChanged(int index) {
  ...
  // But we need to handle the fact that createAppearancePage etc add to stack
  // internally Let's verify the order: 0: Appearance (created by
  // createAppearancePage) 1: Library (placeholder) 2: Reader (created by
  // createReaderPage)
  // ...
...
void SettingsView::onThemeChanged(int index) {
  ...
  switch (index) {
  case 0:
    theme = ThemeManager::NordDark;
    break;
  case 1:
    theme = ThemeManager::NordLight;
    break;
  case 2:
    theme = ThemeManager::CatppuccinMocha;
    break;
  case 3:
    theme = ThemeManager::CatppuccinFrappe;
    break;
  case 4:
    theme = ThemeManager::TokyoNight;
    break;
  case 5:
    theme = ThemeManager::Dracula;
    break;
  default:
    theme = ThemeManager::NordDark;
    break;
